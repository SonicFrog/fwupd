 ...
 * is to enable read protection and then doing dfu_target_stm32_read_unprotect().
 *
 * IMPORTANT: This only works on STM32 devices from ST and UC3 devices from Atmel.
 *
 * Return value: %TRUE for success
 **/
gboolean
		       ...
		       GError **error)
{
	...
	/* implemented */
	if (dfu_device_get_protocol (priv->device) == DFU_DEVICE_PROTOCOL_STM32)
		return dfu_target_stm32_mass_erase (target, cancellable, error);
	else if (dfu_device_get_protocol (priv->device) == DFU_DEVICE_PROTOCOL_UC3)
		return dfu_target_uc3_mass_erase (target, cancellable, error);

	/* invalid */
	g_set_error_literal (error,
			     FWUPD_ERROR,
			     FWUPD_ERROR_NOT_SUPPORTED,
			     "only supported for STM32 and UC3 targets");
	return FALSE;
}
...
dfu_target_attach (DfuTarget *target, GCancellable *cancellable, GError **error)
{
	...
	/* implemented */
	if (dfu_device_get_protocol (priv->device) == DFU_DEVICE_PROTOCOL_STM32)
		return dfu_target_stm32_attach (target, cancellable, error);
	if (dfu_device_get_protocol (priv->device) == DFU_DEVICE_PROTOCOL_UC3)
		return dfu_target_uc3_attach (target, cancellable, error);
			   ...
			   GError **error)
{
	DfuTargetPrivate *priv = GET_PRIVATE (target);
	if (dfu_device_get_protocol (priv->device) == DFU_DEVICE_PROTOCOL_STM32) {
		return dfu_target_stm32_upload_element (target,
							address,
							expected_size,
							maximum_size,
							cancellable,
			     ...
			     GError **error)
{
	...
	/* STM32 specific */
	if (dfu_device_get_protocol (priv->device) == DFU_DEVICE_PROTOCOL_STM32) {
		if (!dfu_target_stm32_download_element (target,
							element,
							flags,
							cancellable,
							error))
