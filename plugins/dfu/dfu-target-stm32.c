...
#define DFU_STM32_CMD_GET_COMMAND		0x00
#define DFU_STM32_CMD_SET_ADDRESS_POINTER	0x21
#define DFU_STM32_CMD_ERASE			0x41
#define DFU_STM32_CMD_READ_UNPROTECT		0x92
...
gboolean
dfu_target_stm32_attach (DfuTarget *target, GCancellable *cancellable, GError **error)
{
...
gboolean
dfu_target_stm32_mass_erase (DfuTarget *target,
 ...
 **/
static gboolean
dfu_target_stm32_set_address (DfuTarget *target,
			      guint32 address,
			      GCancellable *cancellable,
			      GError **error)
{
	...
	/* format buffer */
	buf[0] = DFU_STM32_CMD_SET_ADDRESS_POINTER;
...
DfuElement *
dfu_target_stm32_upload_element (DfuTarget *target,
				 guint32 address,
				 gsize expected_size,
				 gsize maximum_size,
				 GCancellable *cancellable,
				 GError **error)
{
	...
	if (!dfu_target_stm32_set_address (target,
				     offset,
				     cancellable,
				     error))
		return NULL;
...
/**
 * dfu_target_stm32_erase_address:
 * @target: a #DfuTarget
 * @address: memory address
 * @cancellable: a #GCancellable, or %NULL
 * @error: a #GError, or %NULL
 ...
 **/
static gboolean
dfu_target_stm32_erase_address (DfuTarget *target,
				guint32 address,
				GCancellable *cancellable,
				GError **error)
{
...
gboolean
dfu_target_stm32_download_element (DfuTarget *target,
				   DfuElement *element,
				   ...
				   GError **error)
{
	...
	for (guint i = 0; i < sectors_array->len; i++) {
		...
		g_debug ("erasing sector at 0x%04x",
			 dfu_sector_get_address (sector));
		if (!dfu_target_stm32_erase_address (target,
						     dfu_sector_get_address (sector),
						     cancellable,
						     error))
			return FALSE;
...
#if 0
static gboolean
dfu_target_stm32_get_commands (DfuTarget *target,
			       GCancellable *cancellable,
			       GError **error)
{
	GBytes *data_in;
	...
	/* invalid */
	if (dfu_device_get_protocol (device) != DFU_DEVICE_PROTOCOL_STM32) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_NOT_SUPPORTED,
				     "only supported for STM32 targets");
		return FALSE;
	}

	/* format buffer */
	buf[0] = DFU_STM32_CMD_GET_COMMAND;
	data_in = g_bytes_new_static (buf, sizeof(buf));
	if (!dfu_target_download_chunk (target, 0, data_in, cancellable, error)) {
...
/**
 * dfu_target_stm32_read_unprotect:
 * @target: a #DfuTarget
 * @cancellable: a #GCancellable, or %NULL
 * @error: a #GError, or %NULL
 *
 ...
 **/
static gboolean
dfu_target_stm32_read_unprotect (DfuTarget *target,
				 GCancellable *cancellable,
				 GError **error)
{
	...
	/* invalid */
	if (dfu_device_get_protocol (priv->device) != DFU_DEVICE_PROTOCOL_STM32) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_NOT_SUPPORTED,
				     "only supported for STM32 targets");
	...
	/* format buffer */
	buf[0] = DFU_STM32_CMD_READ_UNPROTECT;
	memcpy (buf + 1, &address, 4);
	data_in = g_bytes_new_static (buf, sizeof(buf));
	if (!dfu_target_download_chunk (target, 0, data_in, cancellable, error)) {
